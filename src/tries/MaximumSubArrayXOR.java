...
/**
 * Approach: Maximize the xor subarray by using trie data structure to find the binary inverse of current prefix xor 
 * inorder to set the left most unset bits and maximize the value.
 * 
 * 1.Create an empty Trie. Every node of Trie is going to contain two children, for 0 and 1 values of a bit.
   ...
   3.Traverse the given array and do the following for every array element arr[i].
     ...
     c.  Update the result if the value obtained above is more than the current value of the result.
 */
public class MaximumSubArrayXOR {
	...
	// Finds the maximum XOR ending with last number in prefix XOR 'pre_xor' and
	// returns the XOR of this
	// maximum with pre_xor which is maximum XOR ending with last element of
	// pre_xor.
