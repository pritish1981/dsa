...
public class ShortestDistanceInMaze {
	...
	public static int shortestDistance(int[][] maze, int[] start, int[] end) {
     ...
     distance[start[0]][start[1]] = 0; //set start distance to 0
	...
	public static void dfs(int[][] maze, int[] start, int[][] distance) {
		...
		for(int[] dir : dirs) {
			...
			int y = start[1] + dir[1];
			int count = 0;
			while(x >= 0 && y >= 0 && x < maze.length && y < maze[0].length && maze[x][y] == 0) {//while not hitting the wall
				x = x +  dir[0];
				y = y +  dir[1];
				count++;
			}
